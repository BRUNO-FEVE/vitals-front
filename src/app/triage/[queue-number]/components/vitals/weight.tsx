...
interface WeightVitalsProps {
  index: number;
}

export default function WeightVitals({ index }: WeightVitalsProps) {
  const [progressPercentage, setProgressPercentage] = useState(0);
  const warning = false;
  const { next, currentIndex } = useQuiz();
  const isActive = currentIndex === index;

  console.log("WeightVitals rendered with:", { index, isActive });
  ...
  return (
    <div className="w-full h-full flex flex-row">
      <div
        ...
        style={{ backgroundImage: `url(/weight.png)` }}
      >
        <h1
          className={cn(
